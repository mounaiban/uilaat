UILAAT JSON Database Format (Version 0.6)
-----------------------------------------
**Deprecated Format**

This is a brief description and critique of an outgoing version of
the format used by the database files in the JSON Repositories.
...
* ``comments``: section containing zero or more descriptive names for the DB
  ...
  * This section was renamed to ``db-name`` in Version 0.7, to more
    accurately reflect its intended usage.
...
Handling of multiple definitions of the default translation are not
defined.
...
Support for multi-code point single characters, like emoji with skin tone
and hair colour modifiers is not defined.
...
in an array: ``[s, e, off]``. A code point ``x`` of a value from ``s``
to ``e`` inclusive will be transposed to ``x + off``.
...
* One value per range: ``[[s1, e1, s2, e2...], [v1, v2...]]
  A code point between ``s1`` and ``e1`` inclusive will be mapped
  to ``v1``.

* One value for all ranges: ``[[s1, e1, s2, e2...], [v]]``
  Assign a value only to the last range. A code point that is between
  any given ``s`` and ``e`` inclusive will be mapped to ``v``.

Note that there are two arrays wrapped within an outer array. The
first sub-array contains the target ranges, while the second contains
...
``[regex, repl]``. Text matching the regular expression ``regex`` will
be replaced with ``repl``.

The ``regex`` argument must be a string, while ``repl`` may be a string
or ``null``.
...
~~~~~~~~~~~~~~~~~~~~~~
When multiple replacements translations are wrapped in an outer array,
the replacement at index zero becomes the preferred replacement, and
...
trans-include
*************
The ``trans-include`` section contains a single array of names of other
DBs within the same repository from which all translations will be imported.
...
~~~~~~~~~~~~~~~~~~~~~~~~~
When a DB with reverse-trans applied is imported, the reversal is
be performed only *after* importing.

This behaviour was changed in Version 0.7, as it was found to be
disruptive and limiting.

Order of Precedence of Inclusions
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
If a mapping is defined in both the current and a DB imported using
``trans-include``, only the current DB's mapping is used.

Order of Precedence of Mappings
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Mappings are sorted from embedded regexes. The application decides which
will be applied first. All mappings must be applied in one go, but the
the application must decide when (and in what order) regexes are applied.
...
The phrase ``13bl🜁ckc🜁🝁s spri🝁espiri🝁@moun🜁iban.com`` may be processed
in two ways.

If the regex is performed first:

1. ``13bl🜁ckc🜁🝁s spri🝁espiri🝁@moun🜁iban.com`` is shortened to
   ``13bl🜁ckc🜁🝁s 🙊`` by the regex

2. ``13bl🜁ckc🜁🝁s 🙊`` is then translated to ``１３ｂｌaｃｋｃatｓ 🙊``

If the regex is performed last:

1. ``13bl🜁ckc🜁🝁s spri🝁espiri🝁@moun🜁iban.com`` is translated to
   ``１３ｂｌaｃｋｃatｓ ｓｐｒｉtｅｓｐｉｒｉt＠ｍｏｕｎaｉｂａｎ．ｃｏｍ``

2. The regex has no effect because it only targets the at sign ``@`` at
   ``U+0040`` and not the fullwidth ``＠`` at ``U+FF20``.

Also note how the characters ``a`` and ``t`` were missed by the ``sfjw``
mapping because all translations were performed at once.

Version 0.7 added a means of breaking down the translation process into
multiple passes for a more thorough translation.
